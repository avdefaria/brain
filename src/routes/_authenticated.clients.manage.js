...
import { Search, UserPlus, Eye, UserMinus } from "lucide-react";
      ...
      <Card className="border-[#E4E6F0] shadow-sm">
        <CardHeader className="border-b border-[#E4E6F0] bg-[#F7F8FC]/50">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div className="relative w-full md:w-96">
              ...
              <Input placeholder="Buscar cliente..." className="pl-10 border-[#E4E6F0] focus-visible:ring-[#3D4FE8]" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)}/>
            </div>
        ...
        <CardContent className="p-0">
          <Table>
            ...
            <TableBody>
              {mockClients.map((client) => (<TableRow key={client.id} className="border-[#E4E6F0] hover:bg-[#F7F8FC]/50">
                  ...
                  <TableCell>
                    <div className="flex items-center gap-2">
                      <div className="h-2 w-24 bg-[#E4E6F0] rounded-full overflow-hidden">
                        <div className={cn("h-full rounded-full", client.health > 80 ? "bg-[#22C55E]" : client.health > 50 ? "bg-[#F5A524]" : "bg-[#EF4444]")} style={{ width: `${client.health}%` }}/>
                      </div>
                      <span className="text-xs font-bold text-[#0E0E16]">{client.health}</span>
                    </div>
                  </TableCell>
                  <TableCell>
                    <span className={cn("px-3 py-1 rounded-full text-[10px] font-bold uppercase", client.risk === "low" ? "bg-green-100 text-green-600" : client.risk === "medium" ? "bg-orange-100 text-orange-600" : "bg-red-100 text-red-600")}>
