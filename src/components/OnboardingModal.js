...
import { Rocket, Users, UserPlus, ChevronRight, CheckCircle2, X } from "lucide-react";
      ...
      <DialogContent className="sm:max-w-[500px] border-[#E4E6F0] p-0 overflow-hidden rounded-2xl">
        <div className="p-8 space-y-6">
          <div className="flex items-center justify-between">
            ...
            <button onClick={() => setOpen(false)} className="text-[#8A8FA3] hover:text-[#0E0E16] transition-colors">
              <X className="h-5 w-5"/>
            </button>
          </div>
          ...
          <div className="py-6 space-y-6">
            ...
            return (<div key={s.id} className={cn("flex items-start gap-4 p-4 rounded-xl border transition-all duration-300", isCurrent ? "border-[#3D4FE8] bg-[#3D4FE8]/5 shadow-sm" : "border-transparent opacity-60")}>
                  <div className={cn("h-8 w-8 rounded-full flex items-center justify-center shrink-0", isCompleted ? "bg-[#22C55E] text-white" : isCurrent ? "bg-[#3D4FE8] text-white" : "bg-[#F7F8FC] text-[#8A8FA3]")}>
                    {isCompleted ? <CheckCircle2 className="h-5 w-5"/> : <Icon className="h-4 w-4"/>}
                  </div>
                  <div className="space-y-1">
                    <h4 className={cn("text-sm font-bold", isCurrent ? "text-[#0E0E16]" : "text-[#8A8FA3]")}>
                      {s.title}
                    </h4>
                    {isCurrent && (<p className="text-xs text-[#8A8FA3] leading-relaxed animate-in fade-in slide-in-from-top-1 duration-500">
                        {s.description}
                      </p>)}
                  </div>
                </div>);
          ...
          <div className="flex items-center justify-between pt-4">
            <Button variant="ghost" className="text-[#8A8FA3] hover:text-[#0E0E16]" onClick={() => setOpen(false)}>
              Pular tudo
            </Button>
            <Button className="bg-[#3D4FE8] hover:bg-[#3D4FE8]/90 rounded-full px-8" onClick={() => step < steps.length ? setStep(step + 1) : handleComplete()}>
